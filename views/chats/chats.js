...
async function isAdmin(groupId){
    try {
        let response = await axios.get(`http://54.173.238.58:5000/group/isAdmin/${groupId}`  , {headers:{"Authorization" : token}})
...
async function getMessage(groupId){
    // setInterval(async ()=>{
        ...
        try{
            const response = await axios.get(`http://54.173.238.58:5000/chats/getMessages/${groupId}?msg=${lastId}`,{headers:{"Authorization":token}})
...
async function sendmessage(e){
    ...
    try{
        ...
        const response = await axios.post(`http://54.173.238.58:5000/chats/send-message/${groupId}`,message,{headers:{"Authorization":token}})
...
async function getUsers(groupId){
    ...
    try {
        let response =  await axios.get(`http://54.173.238.58:5000/group/fetch-users/${groupId}`  , {headers:{"Authorization" : token}})
        console.log(response.data);
...
async function removeUser(userId){
    ...
    console.log(details )
    try {
        let response = await axios.post(`http://54.173.238.58:5000/group/remove-user` ,details  , {headers:{"Authorization" : token}})
...
async function makeAdmin(userId){
    ...
    console.log(details )
    try {
        let response = await axios.post(`http://54.173.238.58:5000/group/makeAdmin` ,details  , {headers:{"Authorization" : token}})
...
async function removeAdmin(userId){
    ...
    console.log(details )
    try {
        let response = await axios.post(`http://54.173.238.58:5000/group/removeAdmin` ,details  , {headers:{"Authorization" : token}})
        console.log(response);
        alert('removed admin');
...
document.getElementById('form-group').onsubmit = async function(e){
    ...
    try {
        let response = await axios.post(`http://54.173.238.58:5000/group/addUser`  ,details ,  {headers:{"Authorization" : token}})
